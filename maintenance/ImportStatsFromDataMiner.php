 ...
 * Curse Inc.
 * Cheevos
 * Imports data from DataMiner tables into the Cheevos service.
 *
...
class ImportStatsFromDataMiner extends Maintenance {
	/**
	 * Constructor
	 *
	 * @access	public
	...
	public function __construct() {
		parent::__construct();
		$this->addDescription('Imports data from DataMiner tables into the Cheevos service.  Should only ever be run once.');
	...
	public function execute() {
		...
		if (MASTER_WIKI !== true) {
			...
			$cache->set('ImportStatsFromDataMiner', 0);
		}

		$start = intval($cache->get('ImportStatsFromDataMiner'));
		...
		for ($i = $start; $i <= $total; $i = $i + 1000) {
			...
			while ($row = $result->fetchRow()) {
				...
				$cache->set('ImportStatsFromDataMiner', $i);
			}
		}
	}
}

$maintClass = 'ImportStatsFromDataMiner';
