			...
			if (
				...
				if (!$achievement->isDeleted()) {
					...
					<div class='p-achievement-admin'>
						<span class='p-achievement-delete'><a href='{$manageAchievementsURL}/".($achievement->isChild() ? 'revert' : 'delete')."?aid={$achievement->getId()}' class='mw-ui-button'>".wfMessage(($achievement->isChild() ? 'revert_custom_achievement' : 'delete_achievement'))->escaped()."</a></span>
