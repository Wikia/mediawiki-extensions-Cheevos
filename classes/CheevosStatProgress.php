...
class CheevosStatProgress extends CheevosModel {
	...
	public function __construct( array $data = null ) {
		...
		$this->container['streak_achievement_id'] = isset( $data['streak_achievement_id'] ) &&
													is_int( $data['streak_achievement_id'] ) ?
														$data['streak_achievement_id'] : 0;
		$this->container['count'] = isset( $data['count'] ) && is_int( $data['count'] ) ? $data['count'] : 0;
		$this->container['last_incremented'] = isset( $data['last_incremented'] ) &&
											   is_int( $data['last_incremented'] ) ? $data['last_incremented'] : 0;
	}
}
