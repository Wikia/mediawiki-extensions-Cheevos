...
class TemplateManageAchievements {
	...
	public function achievementsList( array $achievements, array $categories, array $revertHints ): string {
		...
		$HTML .= "<script type=\"text/javascript\"> window.achievementsList = " . json_encode( $jsList ) . ";
 				  </script>";
		...
		<div class='button_bar'>
			<div class='buttons_left'>
				<fieldset>
					...
					<input id='search_button' type='button' value='" . wfMessage( 'list_search' ) .
					 "' class='mw-ui-button mw-ui-progressive' />
					<input id='search_reset_button'
					type='button'
					value='" . wfMessage( 'list_reset' ) . "'
		...
		if ( count( $achievements ) ) {
			...
			foreach ( $categories as $categoryIndex => $category ) {
				...
				foreach ( $achievements as $achievementId => $achievement ) {
					...
					$categoryHTML[$categoryId] .= TemplateAchievements::achievementBlockRow(
						$achievement,
						true,
						[],
						$achievements,
				...
				if ( !empty( $categoryHTML[$categoryId] ) ) {
					...
					class='achievement_category_select" . ( $firstCategory ? ' begin' : '' ) .
							 "' data-slug='{$category->getSlug()}'>{$category->getTitle()}
					</li>";
					$firstCategory = false;
				}
			}
	 ...
	 * Achievements Form
	 *
	 * @param CheevosAchievement $achievement of achievement information.
	...
	public function achievementsForm(
			...
			<form
				...
				<fieldset>
					...
					<input
						...
						type='text'
						maxlength='50'
						placeholder='" . wfMessage( 'achievement_name_helper' )->escaped() . "'
					...
					<input
						...
						type='text'
						maxlength='150'
						placeholder='" . wfMessage( 'achievement_description_helper' )->escaped() . "'
				...
				'' ) . "
					...
					<input
						...
						type='text'
						maxlength='30'
						placeholder='" . wfMessage( 'achievement_category_helper' )->escaped() . "'
						value='" . htmlentities( $category->getName(), ENT_QUOTES ) . "'/>";
		...
		$HTML .= ( isset( $errors['image'] ) ? '<span class="error">' . $errors['image'] . '</span>' : '' ) . "
			...
			<input
				...
				name='points'
				type='text'
				value='" . htmlentities( $achievement->getPoints(), ENT_QUOTES ) . "' /><br/>

			<input
			id='secret'
			name='secret'
			type='checkbox'
			value='1'" . ( $achievement->isSecret() ? " checked='checked'" : null ) . "/>
				<label for='secret'>" .
				 wfMessage( 'secret_achievement' )->escaped() .
				 "<div class='helper_mark'><span>" .
				 wfMessage( 'secret_help' )->escaped() .
				 "</span></div></label><br/>";
		if ( $dsSiteKey === 'master' ) {
			$HTML .= "
			<input
				...
				name='global'
				type='checkbox'
				value='1'" . ( $achievement->isGlobal() ? " checked='checked'" : null ) . "/>
				<label for='global'>" .
					 wfMessage( 'global_achievement' )->escaped() .
					 "<div class='helper_mark'>
						<span>" . wfMessage( 'global_help' )->escaped() . "</span>
			</div></label><br/>
			<input
				id='protected'
				name='protected'
				type='checkbox'
				value='1'" . ( $achievement->isProtected() ? " checked='checked'" : null ) . "/>
				<label for='protected'>" . wfMessage( 'protected_achievement' )->escaped() .
					 "<div class='helper_mark'><span>" . wfMessage( 'protected_help' )->escaped() .
					 "</span></div></label><br/>
			<input
				id='special'
				name='special'
				type='checkbox'
				value='1'" . ( $achievement->getSpecial() ? " checked='checked'" : null ) . "/><label for='special'>" .
					 wfMessage( 'special_achievement' )->escaped() . "<div class='helper_mark'><span>" .
					 wfMessage( 'special_help' )->escaped() . "</span></div></label><br/>
			<input id='show_on_all_sites' name='show_on_all_sites' type='checkbox' value='1'" .
					 ( $achievement->getShow_On_All_Sites() ? " checked='checked'" : null ) .
					 "/><label for='protected'>" . wfMessage( 'show_on_all_sites_achievement' )->escaped() .
					 "<div class='helper_mark'><span>" . wfMessage( 'show_on_all_sites_help' )->escaped() .
					 "</span></div></label><br/>";
		...
		if ( $user->isAllowed( 'edit_meta_achievements' ) ) {
			...
			<label class='label_above'>" . wfMessage( 'criteria_stats' )->escaped() . "<div class='helper_mark'>
			...
			$HTML .= "</div>
				...
				<label class='label_above'>" . wfMessage( 'criteria_value' )->escaped() .
					 "<div class='helper_mark'><span>" . wfMessage( 'criteria_value_help' ) .
					 "</span></div></label>
				<input name='criteria_value' type='text' value='" . ( $criteria['value'] ?? '' ) . "' />

				<label class='label_above'>" . wfMessage( 'criteria_streak' )->escaped() .
					 "<div class='helper_mark'><span>" . wfMessage( 'criteria_stats_help' ) .
					 "</span></div></label>
			...
			$HTML .= "</select>
				...
				<label class='label_above'>" . wfMessage( 'criteria_streak_progress_required' )->escaped() .
					 "<div class='helper_mark'><span>" . wfMessage( 'criteria_streak_progress_required_help' ) .
					 "</span></div></label>
				<input name='criteria_streak_progress_required' type='text' value='" .
					 ( $criteria['streak_progress_required'] ?? '' ) .
					 "' />

				<label class='label_above'>" . wfMessage( 'criteria_streak_reset_to_zero' )->escaped() .
					 "<div class='helper_mark'><span>" . wfMessage( 'criteria_streak_reset_to_zero_help' ) .
					 "</span></div></label>
				...
				<input id='date_range_start_datepicker' data-input='date_range_start' type='text' value='" .
					 (
						 isset( $criteria['date_range_start'] ) &&
						 $criteria['date_range_start'] > 0 ?
							 htmlentities(
				...
				<label for='date_range_end' class='label_above'>" .
					 ...
					 "</label>
				<input
					id='date_range_end_datepicker'
					data-input='date_range_end'
					type='text'
					value='" . (
						isset( $criteria['date_range_end'] ) &&
						$criteria['date_range_end'] > 0 ? htmlentities(
							gmdate( 'Y-m-d', $criteria['date_range_end'] ),
							ENT_QUOTES ) : ''
						) . "'/>
				<input
					id='date_range_end'
			...
			if ( $dsSiteKey === 'master' ) {
				$HTML .= "
				<label class='label_above'>" .
						 wfMessage( 'criteria_per_site_progress_maximum' )->escaped() .
						 "<div class='helper_mark'><span>" .
						 wfMessage( 'criteria_per_site_progress_maximum_help' ) .
						 "</span></div></label>
				<input
					name='criteria_per_site_progress_maximum'
					type='text'
					value='" . (
						isset( $criteria['per_site_progress_maximumd'] ) ?
							$criteria['per_site_progress_maximum'] : ''
					) . "' />";
			}

			$HTML .= "
				<label class='label_above'>" .
					 wfMessage( 'criteria_category_id' )->escaped() .
					 "<div class='helper_mark'><span>" .
					 wfMessage( 'criteria_category_id_help' ) .
					 "</span></div></label>
		...
		$HTML .= "
			...
			<fieldset class='submit'>
				...
				<button id='wiki_submit' name='wiki_submit' class='mw-ui-button mw-ui-progressive'>" .
				 wfMessage( 'achievement_save' )->escaped() .
				 "</button>
			</fieldset>
		</form>";
	...
	public function awardForm( $form, $achievements ): string {
		...
		$HTML .= "
				...
				<button name='do' type='submit' value='award'>" .
				 wfMessage( 'award' )->escaped() .
				 "</button><button name='do' type='submit' value='unaward'>" .
				 wfMessage( 'unaward' )->escaped() . "</button>
			</fieldset>
		</form>";
